            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # In CoPrompt these visual-context tokens come from the
                        # text branch, so each deeper visual block receives
                        # prompt tokens shared from text to vision.
...
class VisionTransformer_MaPLe(nn.Module):
    def __init__(
        ...
        self.prompt_nctx = design_details["maple_length"]
        self.transformer = Transformer(
    ...
    def embed_tokens(self, x: torch.Tensor, shared_ctx):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        if self.VPT_shallow:
            visual_ctx = shared_ctx.expand(x.shape[0], -1, -1).type_as(x)
            x = torch.cat([x, visual_ctx], dim=1)
        else:
            assert self.prompt_till_layer_visual == 0
